...
function parseStatus(result) {
  ...
  // NB: 'capacity' is intentionally not a SOC key - on Bat.GetStatus it is the
  // battery's capacity in Wh (e.g. 5120), not a percentage, and would get
  // clamped to 100% by _applyStatus() instead of being caught by the
  // permille correction below.
  const socKeys = ['bat_soc', 'battery_soc', 'soc', 'batterySoc', 'bms_soc', 'battery_level', 'batteryLevel'];
...
function broadcastAddresses() {
  ...
  for (const list of Object.values(os.networkInterfaces())) {
    for (const address of list || []) {
      ...
      const broadcast = ipOctets
        .map((octet, index) => (octet & maskOctets[index]) | (~maskOctets[index] & 255))
        .join('.');
...
class MarstekClient {
   ...
   *
   * The back-off is capped (see MAX_RETRY_DELAY): Homey aborts a flow card
   * after 30s, and this request() is normally followed by a verification
   * read, so an uncapped `retryDelay * attempt` can burn the whole budget on
   * retries alone and fail the flow card even though the battery did switch.
   */
  async request(method, params = {}, { timeout, attempts = 3, retryDelay = 1500 } = {}) {
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      try {
        ...
        if (attempt < attempts) {
          const delay = Math.min(retryDelay * attempt, MAX_RETRY_DELAY);
  ...
  async setMode(mode, options = {}) {
    const known = MODES.find((candidate) => candidate.toLowerCase() === String(mode || '').toLowerCase());
    if (!known) {
      throw new Error(`Onbekende modus "${mode}" (verwacht: ${MODES.join(', ')})`);
    }

    const { timeout, attempts, retryDelay, ...configOptions } = options;
    const config = Object.assign({ mode: known }, buildModeConfig(known, configOptions));
